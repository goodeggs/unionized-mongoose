...
describe 'random field generation', ->
  ...
  describe '`factory` callbacks', ->
    before ->
      Model = mongoose.model 'Kitten4', mongoose.Schema
        breed: { type: String, factory: (-> 'Tabby'), required: true }
        furColor: { type: String, factory: (-> 'red' ) }
        furStyle: { type: String, factory: (-> undefined ), required: true }
        paws: [
          lastTrimmed: { type: Date, factory: (-> new Date('2000-01-01 12:00')) }
        ]
    ...
    it 'are used even if attribute is not required', ->
      expect(@instance).to.have.property 'furColor', 'red'

    it 'can return `undefined` to omit an attribute', ->
      expect(@instance).not.to.have.property 'furStyle'

    it 'are used in array elements', ->
      expect(@instance.paws).to.have.length.of.at.least 1
      expect(@instance.paws[0]).to.have.property 'lastTrimmed'
      expect(@instance.paws[0].valueOf()).to.equal 946756800000
  ...
  describe 'an instance generated with inputs', ->
    ...
    it 'inputs override `factory` callback', ->
